...
void printCFG(const std::string& inputFile) {
    ...
    auto symtabCS = std::make_unique<ParseAPI::SymtabCodeSource>((char *)inputFile.c_str());
    ...
    for (auto f : all) {
        ...
        for ( auto  b : f->blocks() )  {
            ...
            for (auto& it : b->targets()) {
                std::string s;
                if (it->type() == Dyninst::ParseAPI::CALL) s = " [color=blue]";
                else if (it->type() == Dyninst::ParseAPI::RET) s = " [color=green]";

                edgeoutput << "\t\""
                    << std::hex << it->src()->start()
                    << "\" -> \""
                    << it->trg()->start()
                    << "\"" << s << std::endl;
            }
