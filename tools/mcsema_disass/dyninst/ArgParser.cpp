...
ArgParser::ArgParser(int argc, char **argv)
    : m_appName(argv[0]), m_args(), m_inputFiles(), m_stdDefFiles(),
      m_dumpCfg(false), m_prettyPrintCfg(false), m_outputFiles(),
      m_addExtSyms() {
  ...
  while (!m_args.empty()) {
    ...
    if (curArg.front() == '-') {
      if ((curArg == "-h") || (curArg == "--help"))
        throw std::runtime_error{"-h and --help may not be used in conjunction "
        ...
        std::cerr << "unrecognized command line option: \"" << curArg << "\""
                  << std::endl;
        std::cerr << "use `" << m_appName
                  << " --help` for a list of available options" << std::endl;
...
void ArgParser::printHelp() const {
  std::cout
      ...
      << "  --std-defs <FILE>: read external symbol definitions from <FILE>"
      << std::endl
      << "                     (may be used multiple times)" << std::endl
      << "  --dump-cfg: print the created CFG file to stdout in a "
         "human-readable format"
      << std::endl
      << "  --[no-]pretty-print: pretty-print the CFG file (default: on)"
      << std::endl
      << "                       (only makes sense with --dump-cfg)"
      << std::endl
      << "  -o <FILE>: output resulting CFG into <FILE> (default: none)"
      << std::endl
      << "  --add-ext-sym <DESCRIPTION>: parse <DESCRIPTION> as if it were a "
         "line in a"
      << std::endl
      << "                               symbol definitions file" << std::endl;
}

const std::string &ArgParser::getAppName() const { return m_appName; }
...
const bool ArgParser::getDumpCfg() const { return m_dumpCfg; }

const bool ArgParser::getPrettyPrintCfg() const { return m_prettyPrintCfg; }
