...
class ExternalFunc {
public:
  bool operator<(const ExternalFunc &other) const {
    return m_symbolName < other.m_symbolName;
  }
  ...
  ExternalFunc(const std::string &symbolName, CallingConvention callConv,
               bool hasReturn, bool noReturn, std::int32_t argCount,
               bool isWeak,
