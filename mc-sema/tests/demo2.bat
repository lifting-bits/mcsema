...
if exist "%IDA_PATH%\idaq.exe" (
    ...
    %BIN_DESCEND_PATH%\bin_descend_wrapper.py -d -entry-symbol=start -i=demo_test2.obj
) else (
    echo Using bin_descend to recover CFG
    %BIN_DESCEND_PATH%\bin_descend.exe -d -entry-symbol=start -i=demo_test2.obj
)

%CFG_TO_BC_PATH%\cfg_to_bc.exe -i demo_test2.cfg -driver=demo2_entry,start,raw,return,C -o demo_test2.bc
