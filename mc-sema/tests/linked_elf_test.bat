...
if exist "%IDA_PATH%\idaq.exe" (
    ...
    %BIN_DESCEND_PATH%\bin_descend_wrapper.py -d -func-map=linux_map.txt -i=linked_elf
) else (
    echo Using bin_descend to recover CFG
    %BIN_DESCEND_PATH%\bin_descend.exe -d -func-map=linux_map.txt -i=linked_elf
)

%CFG_TO_BC_PATH%\cfg_to_bc.exe -i linked_cfg -driver=mcsema_start,_start,0,return,C -o demo_linked_elf.bc
