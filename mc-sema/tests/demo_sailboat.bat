...
if exist "%IDA_PATH%\idaq.exe" (
    ...
    %BIN_DESCEND_PATH%\bin_descend_wrapper.py -d -func-map=sailboat.txt,"%STD_DEFS%" -entry-symbol=_keycomp -i=sailboat.obj
) else (
    echo Using bin_descend to recover CFG
    %BIN_DESCEND_PATH%\bin_descend.exe -d -func-map=sailboat.txt,"%STD_DEFS%" -entry-symbol=_keycomp -i=sailboat.obj
)

%CFG_TO_BC_PATH%\cfg_to_bc.exe -i sailboat.cfg -driver=sailboat,_keycomp,1,return,C -o sailboat.bc
