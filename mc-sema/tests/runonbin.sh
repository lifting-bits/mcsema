...
then
    ...
    echo "${BIN_DESCEND_PATH}/bin_descend_wrapper.py -march=x86-64 -d -func-map=${STD_DEFS} -i=${BINF} >> out.txt"
    ${BIN_DESCEND_PATH}/bin_descend_wrapper.py -entry-symbol=main -march=x86-64 -d -func-map=${STD_DEFS} -i=${BINF} >> out.txt
else
