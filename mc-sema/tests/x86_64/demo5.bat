...
if exist "%IDA_PATH%\idaq.exe" (
    ...
    %BIN_DESCEND_PATH%\bin_descend_wrapper.py -d -func-map="%STD_DEFS%" -entry-symbol=_foo -i=demo_test5.obj
) else (
    echo Using bin_descend to recover CFG
    %BIN_DESCEND_PATH%\bin_descend.exe -march=x86-64 -d -func-map="%STD_DEFS%" -entry-symbol=foo -i=demo_test5.obj
)

%CFG_TO_BC_PATH%\cfg_to_bc.exe -march=x86-64 -i demo_test5.cfg -driver=demo5_entry,foo,1,return,C -o demo_test5.bc
