...
if exist "%IDA_PATH%\idaq.exe" (
    ...
    %BIN_DESCEND_PATH%\bin_descend_wrapper.py -d -func-map="%STD_DEFS%" -entry-symbol=_checkFn -i=demo_test7.obj
) else (
    echo Using bin_descend to recover CFG
    %BIN_DESCEND_PATH%\bin_descend.exe -d -func-map="%STD_DEFS%" -entry-symbol=_checkFn -i=demo_test7.obj
)

%CFG_TO_BC_PATH%\cfg_to_bc.exe -i demo_test7.cfg -driver=demo7_entry,_checkFn,1,return,C -o demo_test7.bc
