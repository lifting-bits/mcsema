...
DEFINE_bool(disable_adjacent_segment_merging, false,
...
bool RecoverValueDecl(const NativeFunction *func, anvill::ValueDecl &decl,
  ...
  if (remill::IsError(maybe_type)) {
    LOG(ERROR)
        << "Could not parse type in value spec of function "
  ...
  if (cfg_decl.has_memory()) {
    ...
    if (!decl.mem_reg) {
      LOG(ERROR)
          << "Bad register name '" << cfg_decl.memory().register_()
          << "' in function spec for " << func->name << " at " << std::hex
          << func->ea << std::dec;
      return false;
    }
    ...
    if (!decl.reg) {
      LOG(ERROR)
          << "Bad register name '" << cfg_decl.register_()
          << "' in function spec for " << func->name << " at " << std::hex
          << func->ea << std::dec;
      return false;
    }
...
void RecoverFunctionDecl(NativeModule *module,
                         NativeFunction *func,
  ...
  decl.calling_convention = static_cast<llvm::CallingConv::ID>(
      cfg_decl.calling_convention());

  if (!cfg_decl.has_return_stack_pointer()) {
    LOG(ERROR)
        << "Function spec for " << func->name << " at " << std::hex
  ...
  if (!decl.return_stack_pointer) {
    if (cfg_decl.return_stack_pointer().has_memory()) {
      LOG(ERROR)
          << "Function spec for " << func->name << " at " << std::hex
          << func->ea << std::dec << " has an invalid return stack pointer name '"
          << cfg_decl.return_stack_pointer().memory().register_() << "'";
    } else {
      LOG(ERROR)
          << "Function spec for " << func->name << " at " << std::hex
...
NativeModule *ReadProtoBuf(const std::string &file_name,
    ...
    if (ll_func) {
      ...
      if (remill::IsError(maybe_decl)) {
        ...
        if (remill::IsError(maybe_decl_ptr)) {
          LOG(ERROR)
              << remill::GetErrorString(maybe_decl_ptr);
        } else {
  ...
  for (auto v = 0, max_v = cfg.external_vars_size(); v < max_v; ++v) {
    ...
    if (seg_ptr) {
      LOG(ERROR)
          << "Segment '" << seg_ptr->name << "' at " << std::hex << seg_ptr->ea
          << " is being redefined as '" << cfg_extern_var->name()
          << "' at " << var_ea << std::dec;
  ...
  for (auto s = 0, max_s = cfg.segments_size(); s < max_s; ++s) {
    ...
    if (cfg_segment->has_variable_name()) {
      if (name_to_extern_var.count(cfg_segment->variable_name())) {
        LOG(ERROR)
            << "Skipping segment '" << cfg_segment->variable_name()
  ...
  for (const auto &cfg_segment : cfg.segments()) {
    ...
    for (const auto &cfg_xref : cfg_segment.xrefs()) {
      ...
      CHECK(xref->width <= pointer_size)
          ...
          << std::hex << xref->target_ea << " is too wide at "
          << xref->width << " bytes";
  ...
  for (const auto &cfg_segment : cfg.segments()) {
    ...
    if (auto err = module->MapRange(range); remill::IsError(err)) {
      LOG(FATAL)
          << "Unable to map segment " << segment->name
      ...
      while (ea < entry.ea) {
        ...
        for (auto blob_ea = ea; blob_ea < end_ea; blob_ea += kMaxBlobSize) {
          ...
          if (!blobs.empty() &&
            ...
            blobs.push_back(NativeSegment::Entry{
                blob_ea, blob_end_ea, nullptr, blob.release()});
          }
        }
  ...
  for (const auto &cfg_func : cfg.funcs()) {
    ...
    if (auto func_byte = module->FindByte(func->ea); !func_byte.IsExecutable()) {
      LOG(ERROR)
    ...
    for (const auto &cfg_block : cfg_func.blocks()) {
      ...
      auto it = std::unique(block->successor_eas.begin(), block->successor_eas.end());
  ...
  auto num_preserved_reg_sets = cfg.preserved_regs_size() + cfg.dead_regs_size();
  ...
  if (!FLAGS_disable_adjacent_segment_merging) {
    ...
    for (auto i = module->segments.size(); i > 1u; )  {
      ...
      if ((prev_seg->ea + prev_seg->size) == seg->ea &&
        ...
        LOG(WARNING)
            << "Merging adjacent segments " << prev_seg->name << " at "
            << std::hex << prev_seg->ea << " and "
            << seg->name << " at " << seg->ea << std::dec;

        prev_seg->size += seg->size;
        seg->size = 0;

        prev_seg->entries.insert(
  ...
  for (auto &entry : module->ea_to_func) {
    ...
    if (auto ll_func = gModule->getFunction(func->name); ll_func) {
      ...
      if (!ll_func->hasNUsesOrMore(1)) {
        LOG(INFO)
            << "Erasing " << ll_func->getName().str();
        ll_func->eraseFromParent();
      } else {
        LOG(ERROR)
            << "Renaming existing LLVM function " << func->name
...
const NativeBlock *NativeModule::TryGetBlock(
    uint64_t inst_ea, const NativeBlock *curr) const {

  for (uint64_t block_ea = inst_ea, i = 0u;
       block_ea && i < 256u; block_ea -= 1, ++i) {
