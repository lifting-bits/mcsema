...
class incorrect_query : public std::runtime_error {
 public:
  incorrect_query(int code_, const std::string &mssg_)
      : std::runtime_error(std::string("E") + std::to_string(code_) + ": " +
                           mssg_) {}
};
  ...
  Connection(const std::string &db_name,
    ...
    do {
      ...
      if (!detail::sqlite3_configured) {
        ...
        sqlite3_config(SQLITE_CONFIG_LOG, detail::sqlite_error_log_callback,
                       nullptr);
...
class QueryResult : owned_stmt {
 public:
  ...
  std::enable_if_t<std::is_enum_v<Arg>, Arg> _Get() {
  ...
  template <class Arg, int64_t idx>
  std::enable_if_t<detail::is_one_of_v<Arg, sqlite::blob, sqlite::blob_view>,
                   Arg>
  _Get() {
  ...
  template <class Arg, int64_t idx>
  std::enable_if_t<std::is_integral_v<Arg>, Arg> _Get() {
    return sqlite3_column_int64(stmt, idx);
  }

  template <class Arg, int64_t idx>
  std::enable_if_t<std::is_same_v<std::nullopt_t, Arg>, Arg> _Get() {
    return {};
  }

  template <class Arg, int64_t idx>
  std::enable_if_t<detail::is_std_optional_type<Arg>, Arg> _Get() {
  ...
  std::optional<std::tuple<Ts...>> Get() {
    if (static_cast<int>(sizeof...(Ts)) > sqlite3_column_count(stmt)) {
      throw incorrect_query{SQLITE_ERROR,
                            "Get argument count is greater than allowed"};
    }
...
struct CacheBucket {
  ...
  CacheBucket(Connection &connection) : _connection(connection) {}
  ...
  stmt_ptr get_r(std::string_view query_str_view) {
    ...
    sqlite3_stmt *stmt;
    auto ret = sqlite3_prepare_v3(_connection.db_handle, query_str_view.data(),
