...
struct FuncDeclParams : schema::FuncDeclParams,
                        nm_impl<FuncDeclParams>,
                        has_context {
  using has_context::has_context;
};
...
struct Module_ : schema::Module, has_context, id_based_ops_<Module_<Concrete>> {

  using has_context::has_context;

  constexpr static Query q_insert = R"(insert into modules(name) values (?1))";
  ...
  auto Get(const std::string &name) {
    const static std::string q_user_get =
        "SELECT rowid FROM modules WHERE name = ?1";
...
struct bb_mixin : id_based_ops_<Self>, has_ea<Self> {};
...
struct BasicBlock_ : schema::BasicBlock,
  ...
  std::string_view data(int64_t id) {
    ...
    auto c_data =
        this->_ctx->cache
            .template Find<MemoryRange, MemoryRange_<MemoryRange>::q_data>(
                mr_rowid);
...
struct Segment_ : schema::Segment,
  ...
  auto _insert(uint64_t ea, uint64_t size,
               const typename Concrete::Flags &flags, const std::string &name,
               int64_t memory_rowid) {

    return this->insert(ea, size, flags.read_only, flags.is_external,
                        flags.is_exported, flags.is_thread_local, name,
                        memory_rowid);
  }
  ...
  void SetFlags(int64_t id, const typename Concrete::Flags &flags) {
    ...
    this->_ctx->db.template query<q_set_flags>(
        id, flags.read_only, flags.is_external, flags.is_exported,
...
struct PreservedRegs_ : schema::PreservedRegs,
  ...
  int64_t insert(const typename Concrete::Ranges &ranges,
                 const typename Concrete::Regs &regs, bool is_alive,
                 int64_t module_rowid) {
...
struct FuncDecl_ : schema::FuncDecl,
  ...
  auto GetParams(int64_t id, CtxPtr &full_ctx) ->
      typename Concrete::ValueDecls {
  ...
  c_data_t GetData(int64_t id, CtxPtr &full_ctx) {
    auto [ret_addr_, ret_sp_, is_v, is_n, cc] =
        *(id_ops::get(id)
...
struct dispatch<Segment> {
  ...
  using data_fields =
      util::TypeList<uint64_t, uint64_t, bool, bool, bool, bool>;
};
...
struct dispatch<ExternalVar> {
  ...
  using data_fields =
      util::TypeList<uint64_t, std::string, uint64_t, bool, bool>;
};
...
template <typename Entry>
WeakDataIterator<Entry>::WeakDataIterator(Impl_t &&init)
    : impl(std::move(init)) {}
...
template <typename Entry>
WeakObjectIterator<Entry>::WeakObjectIterator(Impl_t &&init)
    : impl(std::move(init)) {}
...
Workspace::Workspace(const std::string &name)
    : _ctx(std::make_shared<Context>(name)) {}

void Workspace::CreateSchema() {
  constexpr static Query q_pragmas = R"(PRAGMA foreign_keys = ON)";
...
Function Workspace::AddFunction(const Module &module, uint64_t ea,
                                bool is_entrypoint) {
  return {Function_{_ctx}.insert(module._id, ea, is_entrypoint), _ctx};
}

BasicBlock Workspace::AddBasicBlock(const Module &module, uint64_t ea,
                                    std::optional<uint64_t> size,
                                    const MemoryRange &range) {
  return {BasicBlock_{_ctx}.insert(module._id, ea, size, range._id), _ctx};
}

MemoryRange Workspace::AddMemoryRange(const Module &module, uint64_t ea,
                                      uint64_t size, std::string_view data) {
...
ValueDecl Workspace::AddValueDecl(const std::string &type, maybe_str reg,
                                  maybe_str name,
                                  std::optional<MemoryLocation> mem_loc) {
  if (mem_loc) {
    return {impl_t<ValueDecl>(_ctx).insert(type, reg, name, mem_loc->_id),
            _ctx};
  }
...
FuncDecl Workspace::AddFuncDecl(const ValueDecl &ret_address,
                                ...
                                bool is_variadic, bool is_noreturn,
                                CallingConv cc) {
  FuncDecl out{
...
BasicBlock Module::AddBasicBlock(uint64_t ea, std::optional<uint64_t> size,
                                 const MemoryRange &mem) {
...
SymbolTableEntry Module::AddSymbolTableEntry(const std::string &name,
  ...
  return {SymbolTableEntry_{_ctx}.insert(name, _id,
                                         static_cast<unsigned char>(type)),
          _ctx};
}

ExternalFunction
Module::AddExternalFunction(uint64_t ea, const SymbolTableEntry &name,
                            CallingConv cc, bool has_return, bool is_weak) {
  return {ExternalFunction_{_ctx}.insert(ea, static_cast<unsigned char>(cc),
                                         name._id, _id, has_return, is_weak),
          _ctx};
}

GlobalVar Module::AddGlobalVar(uint64_t ea, const std::string &name,
                               uint64_t size) {
  return {impl_t<GlobalVar>(_ctx).insert(ea, name, size, _id), _ctx};
}

ExternalVar Module::AddExternalVar(uint64_t ea, const std::string &name,
                                   uint64_t size, bool is_weak,
                                   bool is_thread_local) {
  return {impl_t<ExternalVar>(_ctx).insert(ea, name, size, is_weak,
                                           is_thread_local, _id),
          _ctx};
}
...
PreservedRegs Module::AddPreservedRegs(const PreservedRegs::Ranges &ranges,
                                       ...
                                       bool is_alive) {
  return {impl_t<mcsema::ws::PreservedRegs>(_ctx).insert(ranges, regs, is_alive,
                                                         _id),
          _ctx};
}
...
#define DEF_WOBJ_IT(who, what_obj, method) \
  WeakObjectIterator<what_obj> who::method() { \
    ...
    return {std::make_unique<details::ObjectIterator_impl>(std::move(result), \
                                                           _ctx)}; \
  }
...
WeakObjectIterator<BasicBlock> Module::OrphanedBasicBlocks() {
  ...
  return {
      std::make_unique<details::ObjectIterator_impl>(std::move(result), _ctx)};
}
...
std::optional<FuncDecl> Function::GetFuncDecl() {
  auto maybe_spec =
      FuncSpec(_ctx).GetOthers_r<schema::Function>(_id).GetScalar<int64_t>();
...
CodeXref BasicBlock::AddXref(uint64_t ea, uint64_t target_ea,
                             OperandType op_type) {
  return {CodeXref_{_ctx}.insert(ea, target_ea, _id,
                                 static_cast<unsigned char>(op_type), nullptr,
                                 nullptr),
          _ctx};
}
...
CodeXref BasicBlock::AddXref(uint64_t ea, uint64_t target_ea,
                             OperandType op_type, const SymbolTableEntry &name,
  ...
  return {CodeXref_{_ctx}.insert(ea, target_ea, _id,
                                 static_cast<unsigned char>(op_type), *mask,
                                 name._id),
          _ctx};
}
...
DataXref Segment::AddXref(uint64_t ea, uint64_t target_ea, uint64_t width,
                          FixupKind fixup) {
  return {DataXref_{_ctx}.insert(ea, width, target_ea, _id,
                                 static_cast<unsigned char>(fixup), nullptr),
          _ctx};
}
...
DataXref Segment::AddXref(uint64_t ea, uint64_t target_ea, uint64_t width,
  ...
  return {DataXref_{_ctx}.insert(ea, width, target_ea, _id,
                                 static_cast<unsigned char>(fixup), name._id),
          _ctx};
}
...
Segment MemoryRange::AddSegment(uint64_t ea, uint64_t size,
...
std::optional<FuncDecl> ExternalFunction::GetFuncDecl() {
  auto maybe_spec = ExtFuncSpec(_ctx)
                        .GetOthers_r<schema::ExternalFunction>(_id)
                        .GetScalar<int64_t>();
  return details::Construct::Create<FuncDecl>(maybe_spec, _ctx);
}
...
#define DEFINE_DATA_OPERATOR(Type) \
  Type::data_t Type::operator*() const { \
    ...
    return impl_t<self_t>{_ctx}.c_get<typename self_t::data_t>( \
        _id, data_fields_t<self_t>{}); \
  }

#define DEFINE_FULL_DATA_OPERATOR(Type) \
  Type::data_t Type::operator*() const { \
    using self_t = remove_cvp_t<decltype(this)>; \
    return impl_t<self_t>{_ctx}.c_get_f<typename self_t::data_t>( \
        _id, data_fields_t<self_t>{}, _ctx); \
  }
...
template <typename Self>
void interface::HasSymbolTableEntry<Self>::Name(const SymbolTableEntry &name) {
...
template <typename Self>
std::optional<SymbolTableEntry::data_t>
interface::HasSymbolTableEntry<Self>::Symbol() {
  auto self = static_cast<Self *>(this);
...
template <typename Self>
std::optional<Self> interface::HasEa<Self>::MatchEa(details::CtxPtr &ctx_ptr,
                                                    int64_t module_id,
                                                    uint64_t ea) {
