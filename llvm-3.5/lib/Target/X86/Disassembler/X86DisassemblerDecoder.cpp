...
static int readOperands(struct InternalInstruction* insn) {
  ...
  for (const auto &Op : x86OperandSets[insn->spec->operands]) {
    ...
    insn->operandPosition[loopIndex] = insn->readerCursor - insn->startLocation;
    loopIndex += 1;
...
int llvm::X86Disassembler::decodeInstruction(
  ...
  uint8_t tbtable[] = {0, 0, 0x38, 0x3a, 0xa6, 0xa7};
  ...
  {
      case THREEBYTE_38:
          insn->native_opcode[index] = 0x38;
          ++index;
          insn->native_opcode[index] = tbtable[insn->opcodeType];
          ++index;
          break;
      case THREEBYTE_3A:
          insn->native_opcode[index] = 0x3A;
          ++index;
          insn->native_opcode[index] = tbtable[insn->opcodeType];
          ++index;
          break;
  ...
  insn->native_opcode[index] = insn->opcode;
  ++index;
  if(insn->consumedModRM) {
      insn->native_opcode[index] = insn->modRM;
  } else if(insn->consumedSIB) {
      insn->native_opcode[index] = insn->sib;
  }
