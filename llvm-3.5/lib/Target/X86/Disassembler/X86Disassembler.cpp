...
// Loop through all newly created MCOperand elements
// and attempt to find their offset in the instruction
static void setOffsetForOperands(
        int index,
        unsigned start,
        MCInst &mcInst,
        const InternalInstruction &insn) {

    unsigned ops = mcInst.getNumOperands();
    // no new operands were added
    if(start == ops) {
        return;
    }

    for(unsigned i = start; i < ops; i++) {
        MCOperand &op = mcInst.getOperand(i);
        op.setOffset(insn.operandPosition[index]);
    }
...
static bool translateInstruction(MCInst &mcInst,
  ...
  memset(&mcInst.native_opcode[0], 0, sizeof(mcInst.native_opcode));
  memcpy(&mcInst.native_opcode[0], insn.native_opcode, sizeof(mcInst.native_opcode));
  memcpy(&mcInst.prefixPresent[0], &insn.prefixPresent[0],  sizeof(uint8_t) * 0x100);
  ...
  for (const auto &Op : insn.operands) {
    if (Op.encoding != ENCODING_NONE) {
      unsigned int opscount = mcInst.getNumOperands();

      if (translateOperand(mcInst, Op, insn, Dis)) {
        return true;
      }
 
      setOffsetForOperands(index, opscount, mcInst, insn);
    }
    ++index;
  }
